...
実際の dogfooding targets には
[baskduf/harness_starter_kit_django](https://github.com/baskduf/harness_starter_kit_django)
と [baskduf/today-bus](https://github.com/baskduf/today-bus) が含まれます。
Django target は prompt-first adoption、`/harness update`、failure memory、
effectiveness tracking を検証し、Next.js target は external API work、
deterministic behavior checks、dogfood review gates を実リポジトリで検証します。
